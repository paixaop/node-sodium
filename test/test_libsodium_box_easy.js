var assert = require('assert');
var sodium = require('../build/Release/sodium');


var expected1 = Buffer.from([
    
]);

var expected2 = Buffer.from([
    0x25,0x39,0x12,0x1d,0x8e,0x23,0x4e,0x65,
    0x2d,0x65,0x1f,0xa4,0xc8,0xcf,0xf8,0x80
]);

var expected3 = Buffer.from([    
    0x25,0x39,0x12,0x1d,0x8e,0x23,0x4e,0x65,
    0x2d,0x65,0x1f,0xa4,0xc8,0xcf,0xf8,0x80
]);

var alicesk = Buffer.from([
    0x77, 0x07, 0x6d, 0x0a, 0x73, 0x18, 0xa5,
    0x7d, 0x3c, 0x16, 0xc1, 0x72, 0x51, 0xb2,
    0x66, 0x45, 0xdf, 0x4c, 0x2f, 0x87, 0xeb,
    0xc0, 0x99, 0x2a, 0xb1, 0x77, 0xfb, 0xa5,
    0x1d, 0xb9, 0x2c, 0x2a
]);

var bobpk = Buffer.from([
    0xde, 0x9e, 0xdb, 0x7d, 0x7b, 0x7d, 0xc1,
    0xb4, 0xd3, 0x5b, 0x61, 0xc2, 0xec, 0xe4,
    0x35, 0x37, 0x3f, 0x83, 0x43, 0xc8, 0x5b,
    0x78, 0x67, 0x4d, 0xad, 0xfc, 0x7e, 0x14,
    0x6f, 0x88, 0x2b, 0x4f
]);

var nonce = Buffer.from([
    0x69, 0x69, 0x6e, 0xe9, 0x55, 0xb6,
    0x2b, 0x73, 0xcd, 0x62, 0xbd, 0xa8,
    0x75, 0xfc, 0x73, 0xd6, 0x82, 0x19,
    0xe0, 0x03, 0x6b, 0x7a, 0x0b, 0x37
]);

var message = Buffer.from([
    
]);


describe("libsodium_box_easy", function () {
    it('should encrypt/decrypt', function() {
        
        var c = sodium.crypto_box_easy(message, nonce, bobpk, alicesk);
        assert(c.equals(expected1));

        var plainText = sodium.crypto_box_open_easy(c, nonce, bobpk, alicesk);
        assert(plainText.equals(message));
        
    });

    it('should encrypt null message', function() {
        /* Null message */
    
        var c = sodium.crypto_box_easy(null, nonce, bobpk, alicesk);
        assert(c.equals(expected2));

        var m = sodium.crypto_box_open_easy(c, nonce, bobpk, alicesk);
        assert(m.length == 0);
        assert(m[0] == null);

        c[sodium.randombytes_uniform(sodium.crypto_box_MACBYTES)]++;
        var ret = sodium.crypto_box_open_easy(c, nonce, bobpk, alicesk);
        assert(ret == null);
    }); 
});

