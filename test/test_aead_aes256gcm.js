...
// Convert vectors into objects
var tests = [];
initVctors.forEach(function(v) {
    tests.push({
        key: toBuffer(v[0], 'hex'),
...
describe("AEAD", function() {
    ...
    it('aes256gcm should work for all ' + tests.length + ' test vectors', function(done) {
        tests.forEach(function(test) {
            assert.equal(test.key.length, sodium.crypto_aead_aes256gcm_KEYBYTES);
            assert.equal(test.nonce.length, sodium.crypto_aead_aes256gcm_NPUBBYTES);
            assert.equal(test.mac.length, sodium.crypto_aead_aes256gcm_ABYTES);
